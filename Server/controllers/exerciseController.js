  ...
  const exerciseobj = await knex("exercises")
    ...
    .then((data) => {
      if (data.length !== 0) {
        ...
        res
          .status(202)
          .send(`Exercise with an ID of ${exerciseID} could not be found.`);
      }
    });
    ...
    .then((data) => {
      if (data.length !== 0) {
        ...
        res
          .status(202)
          .send(
            `${aircraftType} with a unit ID of ${unitExerciseID} could not be found.`
          );
      }
    });
  if (exerciseobj !== undefined) {
  ...
  knex("exerciseaircraft")
    ...
    .catch((err) =>
      res
        .status(202)
        .send(
          `${aircraftType} with a unit ID of ${unitExerciseID} could not be found. ${err}`
        )
    );
};
      ...
      if (data.length !== 0) {
        return knex("exercises")
          ...
          .then(() => {
            res
              .status(201)
              .send(`Exercise with ID ${exerciseID} has been updated.`);
          });
      } else {
    ...
    .then((data) => {
      if (data.length !== 0) {
        ...
        res
          .status(202)
          .send(
            `Can't find an entry for ${unit} participating in exercise with ID ${exerciseID}.`
          );
      }
    });
};
    ...
    .then((data) => {
      if (data.length === 0) {
        return knex("unitexercises")
          ...
          .catch((err) => console.log("save unit exercise", err));
      } else {
        ...
        return knex("exerciseaircraft")
          ...
          .then(() => {
            res
              .status(201)
              .send(
                `Created entry for ${aircraftType} in the unit exercise with ID ${unitExerciseID}.`
              );
          });
      } else {
        res
          .status(202)
          .send(
            `There is already an entry for ${aircraftType} in the unit exercise with ID ${unitExerciseID}.`
          );
      }
    });
};
