  ...
  rows.forEach(row => {
    if(row.airCraftType === e.target.name){
      ...
      airframeList.forEach(entry =>{
          if(entry.aircraftName === row.airCraftType && parseInt(row.airCraftAmount) === entry.aircraftNumber){
  ...
  console.log(newExerciseObject.overView.totalPersonal)
    ...
    return (
      <TableContainer component={Paper}>
        <Table sx={{ minWidth: 700 }} aria-label="customized table">
          <TableHead>
            <TableRow>
              ...
              <StyledTableCell align="center">Number of Airframes</StyledTableCell>
              <StyledTableCell align="center">Number of Personnel</StyledTableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {rows.map((row) => (
              <StyledTableRow key={row.airCraftType}>
                ...
                <StyledTableCell align="center"><TextField 
                  inputProps={{min: 0, max: 16, style: { textAlign: 'center' }}} 
        ...
        <Typography variant="body1" >Total Personnel: {totalPersonnel}</Typography>
        </span>
      </TableContainer>
    );
}
